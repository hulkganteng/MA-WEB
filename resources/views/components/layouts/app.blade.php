...
<body class="min-h-screen bg-white text-[#1F1A17] antialiased selection:bg-primary-600 selection:text-white"
      x-data="{
          scrollY: 0,
          scrollProgress: 0,
          updateScroll() {
    ...
    <div x-show="$store.prayer.modalOpen"
        ...
        <div class="relative my-auto box-border max-h-[calc(100dvh-2rem)] w-full max-w-[min(32rem,calc(100vw-2rem))] overflow-x-hidden overflow-y-auto overscroll-contain rounded-3xl border border-white/20 bg-[#006437] text-white shadow-2xl"
            ...
            <div class="relative p-6 sm:p-8">
                ...
                {{-- Live Countdown Card --}}
                <div class="mt-6 rounded-2xl border border-white/20 bg-black/20 p-4 text-center backdrop-blur">
                    <p class="text-xs font-semibold uppercase tracking-wider text-gold-300">Menuju Waktu Sholat Berikutnya</p>
                ...
                <div class="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-3">
                    <template x-for="p in [
                        ...
                        <div class="flex flex-col items-center rounded-xl p-3 text-center transition"
                             :class="$store.prayer.nextPrayerName === p.name ? 'border border-gold-400 bg-gold-400/20 shadow-md font-bold' : 'border border-white/10 bg-white/10'">
                            <span class="text-xs text-primary-100" x-text="p.name"></span>
        ...
        <div class="relative my-auto max-h-[calc(100dvh-2rem)] w-full max-w-2xl overflow-y-auto overscroll-contain rounded-3xl border border-slate-200 bg-white text-[#1F1A17] shadow-2xl"
            ...
            <div class="bg-[#006437] p-6 text-white sm:p-7">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-primary-100">Temukan jurusan & program MA Assa'adah yang sesuai dengan minat dan bakat Anda.</p>
                        </div>
                    </div>
                ...
                <div class="mt-5" x-show="!$store.spmbCalc.result">
                    <div class="flex justify-between text-xs font-semibold text-primary-100">
            ...
            <div class="p-6 sm:p-8">
                ...
                <div x-show="!$store.spmbCalc.result">
                    <template x-for="(q, qIndex) in $store.spmbCalc.questions">
                        <div x-show="$store.spmbCalc.step === (qIndex + 1)" x-transition>
                            <h4 class="text-lg font-bold text-[#1F1A17] sm:text-xl" x-text="q.title"></h4>
                            <p class="mt-1 text-sm text-slate-600" x-text="q.subtitle"></p>
                                ...
                                <template x-for="(opt, oIndex) in q.options">
                                    <button type="button"
                                        ...
                                        <div class="flex items-center gap-3.5">
                                            <span class="flex size-9 items-center justify-center rounded-xl bg-slate-100 text-[#1F1A17] transition group-hover:bg-primary-600 group-hover:text-white font-bold text-sm"
                                                  x-text="String.fromCharCode(65 + oIndex)"></span>
                                            <span class="text-sm font-semibold text-[#1F1A17] group-hover:text-primary-800" x-text="opt.label"></span>
                                        </div>
                ...
                <div x-show="$store.spmbCalc.result" x-transition>
                    <div class="rounded-2xl border border-primary-200 bg-primary-50/80 p-6">
                        ...
                        <div class="mt-5 border-t border-primary-200 pt-4">
                            ...
                            <ul class="mt-2 grid gap-1.5 sm:grid-cols-2 text-xs font-medium text-[#1F1A17]">
                    ...
                    <div class="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <div class="flex flex-wrap items-center gap-2.5">
                            <a href="https://lynk.id/spmb-madah" target="_blank" rel="noopener noreferrer"
                               class="btn-primary font-bold shadow-soft flex items-center gap-2">
                            ...
                            <a :href="$store.spmbCalc.result?.actionUrl" target="_blank" rel="noopener"
                               class="btn-gold font-bold">
                                <x-icon name="message-circle" class="size-4" /> Konsultasi WA
                            </a>
                        </div>
                    </div>
                </div>
            ...
            <div class="max-h-80 overflow-y-auto p-2">
                <template x-for="item in $store.cmdPalette.filteredItems">
                    <a :href="item.url"
                       ...
                       :rel="item.url.startsWith('http') ? 'noopener noreferrer' : null"
    ...
    <aside aria-label="Aksi Cepat" class="fixed bottom-4 right-3 sm:bottom-6 sm:right-6 z-40 flex flex-col items-end gap-2.5 sm:gap-3" x-data="{ expanded: false }">
        ...
        <div x-show="expanded"
             ...
             class="flex flex-col items-end gap-2 max-w-[calc(100vw-2rem)]"
             x-cloak>

            {{-- SPMB Direct Registration Link --}}
            ...
            <button type="button"
                    ...
                    class="group flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3.5 py-2 text-xs font-semibold text-[#1F1A17] shadow-soft transition hover:border-primary-600 hover:text-primary-700">
                <span>Jadwal Sholat Bungah</span>
                <span class="flex size-6 sm:size-7 items-center justify-center rounded-full bg-primary-50 text-primary-700">
                    <x-icon name="clock" class="size-3 sm:size-3.5" />
                </span>
            </button>
            ...
            <button type="button"
                ...
                <span class="flex size-6 sm:size-7 items-center justify-center rounded-full bg-gold-50 text-gold-700">
                    <x-icon name="music" class="size-3 sm:size-3.5" />
                </span>
            </button>
            ...
            <button type="button"
                    ...
                    class="group flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3.5 py-2 text-xs font-semibold text-[#1F1A17] shadow-soft transition hover:border-primary-600 hover:text-primary-700">
                <span x-text="$store.accessibility.isLargeFont ? 'Huruf Normal' : 'Huruf Besar'"></span>
                <span class="flex size-6 sm:size-7 items-center justify-center rounded-full bg-slate-100 font-bold text-xs text-[#1F1A17]">
            ...
            <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $waNum) }}?text={{ $waMsg }}"
               ...
               class="group flex items-center gap-2 rounded-full border border-emerald-500/40 bg-[#25D366] px-3.5 py-2 text-xs font-semibold text-white shadow-lift transition hover:brightness-105">
                <span>Hotline Madrasah</span>
                <span class="flex size-6 sm:size-7 items-center justify-center rounded-full bg-white/20 text-white">
                    <x-icon name="message-circle" class="size-3 sm:size-3.5" />
                </span>
            </a>
        </div>
        ...
        <div class="flex items-center gap-2">
            ...
            <button type="button"
                    ...
                    class="relative flex size-11 sm:size-12 items-center justify-center rounded-full bg-white text-slate-700 shadow-lift border border-slate-200 transition hover:bg-primary-50 hover:text-primary-700"
                    aria-label="Kembali ke atas">
                <svg class="size-11 sm:size-12 -rotate-90 text-primary-600" viewBox="0 0 48 48">
            ...
            <button type="button"
                ...
                <span class="absolute -top-0.5 -right-0.5 flex size-3">
                    ...
                    <span class="relative inline-flex size-3 rounded-full bg-gold-400"></span>
                </span>
                <x-icon name="plus" class="size-5 sm:size-6 transition-transform duration-300" x-show="expanded" />
                <x-icon name="sparkles" class="size-5 sm:size-6 transition-transform duration-300" x-show="!expanded" />
            </button>
        </div>
    </aside>

    {{-- Docked Islamic Audio Player (Responsive for Mobile) --}}
    <div x-show="$store.audioPlayer.isOpen"
         x-transition:enter="transition ease-out duration-300"
         x-transition:enter-start="opacity-0 -translate-y-4 sm:-translate-x-6 scale-95"
        ...
        <div class="flex items-center justify-between border-b border-white/15 pb-3">
            <div class="flex items-center gap-2.5">
                <span class="flex size-8 items-center justify-center rounded-xl bg-gold-400 text-[#1F1A17]">
        ...
        <div class="mt-3 flex items-center justify-between gap-3">
            ...
            <div class="flex items-end gap-1 h-5 px-1">
                <span class="equalizer-bar bg-white" :style="$store.audioPlayer.isPlaying ? '' : 'animation: none; height: 4px'"></span>
                <span class="equalizer-bar bg-gold-400" :style="$store.audioPlayer.isPlaying ? '' : 'animation: none; height: 4px'"></span>
                <span class="equalizer-bar bg-secondary-300" :style="$store.audioPlayer.isPlaying ? '' : 'animation: none; height: 4px'"></span>
            ...
            <div class="flex items-center gap-2">
                ...
                <button type="button" @click="$store.audioPlayer.togglePlay()"
                        class="flex size-8 items-center justify-center rounded-full bg-gold-400 text-[#1F1A17] font-bold hover:bg-gold-300 transition"
