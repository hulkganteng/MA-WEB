<x-layouts.admin title="Guru & Tendik">
    ...
    <form method="GET" class="mt-7 grid gap-3 sm:grid-cols-[minmax(0,1fr)_12rem_12rem_auto]">
        ...
        <div><label for="type" class="sr-only">Jenis tenaga</label><div class="grid grid-cols-[1fr_2rem]"><select id="type" name="type" class="input col-span-full row-start-1 appearance-none !py-2 pr-8"><option value="">Semua jenis</option><option value="guru" @selected(request('type') === 'guru')>Guru</option><option value="tendik" @selected(request('type') === 'tendik')>Tenaga kependidikan</option></select><x-icon name="chevron-down" class="pointer-events-none col-start-2 row-start-1 size-4 place-self-center text-slate-500" /></div></div>
        <div><label for="status" class="sr-only">Status tampil</label><div class="grid grid-cols-[1fr_2rem]"><select id="status" name="status" class="input col-span-full row-start-1 appearance-none !py-2 pr-8"><option value="">Semua status</option><option value="published" @selected(request('status') === 'published')>Tampil di website</option><option value="hidden" @selected(request('status') === 'hidden')>Disembunyikan</option><option value="trash" @selected(request('status') === 'trash')>Sampah</option></select><x-icon name="chevron-down" class="pointer-events-none col-start-2 row-start-1 size-4 place-self-center text-slate-500" /></div></div>
