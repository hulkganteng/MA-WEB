    ...
    public function index(Request $request) {
        ...
        if ($term !== '') {
            $results = collect([
                ...
                'Pengumuman' => Announcement::published()->where(fn ($q) => $q->where('title','like',$like)->orWhere('body','like',$like))->limit(8)->get()->map(fn ($item) => ['title'=>$item->title,'description'=>strip_tags($item->body),'url'=>route('pengumuman.show',$item)]),
